...
import {
  ...
  Flame, Gift, Tag, Trophy, ChevronRight,
} from 'lucide-react'
...
type GsapTarget = string | Element | null | undefined
type Gsap = {
  fromTo: (t: GsapTarget, from: GsapVars, to: GsapVars) => void
  to:     (t: GsapTarget, v: GsapVars) => void
  set:    (t: GsapTarget, v: GsapVars) => void
}

function getGsap(): Gsap | undefined {
  if (typeof window === 'undefined') return undefined
  return (window as unknown as { gsap?: Gsap }).gsap
}
      ...
      >
        ...
        <div ref={stepRef} style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
          ...
          {step === 1 && (
            <div style={{ padding: '24px 24px 8px' }}>
              ...
              <button onClick={() => goTo(2)} style={{ width: '100%', padding: '14px', borderRadius: 14, border: 'none', background: gradient, color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
                Avanti
              </button>
            </div>
          )}
          ...
          {step === 2 && (
            <div style={{ padding: '24px 24px 8px' }}>
              ...
              <button onClick={() => goTo(3)} style={{ width: '100%', padding: '14px', borderRadius: 14, border: 'none', background: gradient, color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
                Avanti
              </button>
            </div>
          )}
          ...
          {step === 3 && (
            <div style={{ padding: '24px 24px 8px' }}>
              ...
              <button onClick={() => goTo(4)} style={{ width: '100%', padding: '14px', borderRadius: 14, border: 'none', background: gradient, color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
                Avanti
              </button>
            </div>
          )}
          ...
          {step === 4 && (
            <div>
              {/* Gradient header with stacked notifications */}
              <div style={{ background: gradient, padding: '32px 24px 20px' }}>
                <div style={{ position: 'relative', height: 100 }}>
                  {DEMO_NOTIFS.map((notif, i) => {
                    const scale   = 1 - (2 - i) * 0.05
                    const offsetY = (2 - i) * 7
                    const opacity = [0.4, 0.7, 1][i]
                    return (
                      <div
                        key={notif.title}
                        id={`ob-s4-n${i}`}
                        style={{
                          position: 'absolute', top: offsetY, left: `${(2 - i) * 3}%`, right: `${(2 - i) * 3}%`,
                ...
                <div id="ob-s4-cta" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      opacity: loading ? 0.75 : 1, transition: 'opacity 150ms',
                    }}
                  >
                    <Bell size={18} />
        ...
        <div style={{ padding: '8px 24px 20px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          {step < 4 ? (
            <button
              onClick={() => goTo(step + 1)}
              style={{ background: 'none', border: 'none', fontSize: 13, color: '#B0B0B0', cursor: 'pointer', padding: '4px 0' }}
            >
              Salta
            </button>
          ) : (
