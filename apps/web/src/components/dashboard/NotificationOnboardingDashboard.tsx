...
const DEMO_NOTIFS = [
  { title: 'Nuova prenotazione 📅', body: 'Luca M. · oggi alle 15:00',      time: 'adesso' },
  { title: 'Cliente a rischio ⚠️',  body: 'Marco non viene da 38 giorni',    time: '2 min fa' },
  { title: 'Report pronto 📊',       body: 'Questa settimana: 14 appuntamenti', time: '1h fa' },
]

const BULLETS = [
  { Icon: CalendarPlus, label: 'Prenotazioni in tempo reale' },
  { Icon: UserX,         label: 'Clienti a rischio churn' },
  { Icon: BarChart2,     label: 'Report settimanale' },
]

async function updatePrefs(prefs: Record<string, boolean>) {
  const db = createClient()
  const { data: { user } } = await db.auth.getUser()
  if (!user) return
  const res = await db.from('profiles').select('notification_preferences').eq('id', user.id).maybeSingle() as unknown as { data: { notification_preferences: Record<string, boolean> } | null }
  const current = res.data?.notification_preferences ?? {}
  await db.from('profiles').update({ notification_preferences: { ...current, ...prefs } }).eq('id', user.id)
}

export function NotificationOnboardingDashboard({ primaryColor, logoUrl, businessName, tenantId }: Props) {
  ...
  async function persistAndClose(accepted: boolean) {
    await animateOut()
    setShow(false)
    updatePrefs({ push_prompted: true, push_accepted: accepted }).catch(console.error)
  }
      ...
      >
        ...
        <div style={{ padding: '24px 24px 28px' }}>
          ...
          <p style={{ margin: '0 0 20px', fontSize: 14, color: '#666666', lineHeight: 1.55 }}>
            Ricevi notifiche istantanee per nuove prenotazioni, cancellazioni e clienti a rischio.
          </p>
