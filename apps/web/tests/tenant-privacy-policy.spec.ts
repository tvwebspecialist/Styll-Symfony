...
test.describe('tenant privacy policy', () => {
  ...
  test('tenant privacy contains required sections, salon identity, routing, retention, analytics, and Silent Churn disclosure', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator('body')).toContainText('Gestisci i tuoi dati')
      await expect(page.locator('body')).toContainText('export JSON tenant-scoped')
      await expect(page.locator('body')).toContainText('richiesta manuale')
      await expect(page.locator('body')).toContainText('contattare il Titolare')
