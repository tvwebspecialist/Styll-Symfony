...
| Tabella | Scopo | Fase |
|---------|-------|------|
| `products` | Catalogo prodotti del tenant. Nome, marca, prezzo vendita, prezzo costo, SKU (opzionale), foto, categoria, attivo sì/no | v1 |
| `product_inventory` | Giacenza per prodotto **per sede**. Quantità attuale, soglia alert scorta bassa | v1 |
| `appointment_products` | Prodotti venduti/usati durante un appuntamento. Quantità, prezzo snapshot al momento della vendita | v1 |
| `inventory_movements` | Log di ogni movimento magazzino: carico (acquisto fornitore), scarico (vendita), rettifica manuale, reso. Audit trail completo | v2 |

### Decisioni chiave
- **Servizi e prodotti restano in tabelle separate**: un servizio è tempo (ha durata), un prodotto è un bene fisico (ha giacenza, prezzo costo, SKU). Nature diverse, tabelle diverse
- **La giacenza è per sede**: Sara ha 2 sedi, la Matt Clay può avere 12 pezzi a Roma Centro e 3 a Roma Nord
- **Il prezzo è snapshotted**: se il barbiere cambia il prezzo della Matt Clay da €18 a €20, gli appuntamenti passati mostrano €18
- **Il cliente non aggiunge prodotti nel booking**: i prodotti li gestisce il barbiere dalla dashboard
- **Il trigger di decremento giacenza** è automatico: quando il barbiere aggiunge un prodotto all'appuntamento, la quantità in `product_inventory` si aggiorna

### Impatto sulle altre aree
- **CRM/Analytics**: la spesa prodotti è tracciata separatamente dalla spesa servizi. Il VIP Score può includere gli acquisti prodotti (chi compra prodotti è più fedele)
- **Dashboard**: alert scorta bassa visibile nel pannello notifiche del barbiere
- **Loyalty**: i punti si calcolano sulla spesa totale (servizi + prodotti) nei template basati su €1 speso

### Posizionamento competitivo

| Competitor | Gestione prodotti | Inventario |
|-----------|------------------|------------|
| **Barberly** | ❌ | ❌ |
| **GlossGenius** | ⚠️ Vendita base | ❌ |
| **Fresha** | ✅ Vendita | ⚠️ Base |
| **Phorest** | ✅ Completa | ✅ Completa ($99+/mese) |
| **Styll** | ✅ Vendita in appuntamento | ✅ Giacenza base v1, completa v2 |

---

## 🗄️ Architettura Database — Schema progettuale

> Decisioni prese durante la progettazione dello schema database con Supabase (PostgreSQL).
> Lo schema è stato progettato per supportare la roadmap v1 → v2 → v3 senza riscritture.

### Decisioni architetturali pre-schema

| # | Tema | Decisione |
|---|------|-----------|
| 1 | **Auth cliente** | Guest può solo prenotare. Account (OTP telefono) sblocca loyalty, gamification, promo. Punti si accumulano sempre, visibili solo con account. Conversione spinta in ogni touchpoint. |
| 2 | **Multi-location** | Tenant = business. Staff assegnato a 1+ sedi via tabella ponte. Vede solo le sue sedi. Owner vede tutto. Clienti appartengono al tenant, non alla sede. |
| 3a | **Multi-servizio** | Tabella ponte `appointment_services`. Prezzo snapshot al momento del booking. |
| 3b | **Pagamento v1** | Tracking offline. No gateway. |
| 3c | **Conferma** | Auto-conferma default, manuale attivabile. |
| 4 | **Orari** | Slot multipli per giorno per staff member + override per eccezioni. |
| 5 | **Review v1** | Link esterno Google Reviews + tracking invio. |
| 6 | **Admin** | Metadata + metriche aggregate. No dati operativi dei tenant. |

### Mappa delle 10 aree funzionali

Il database è organizzato in 10 macro-aree, dalle fondamenta verso le funzionalità più alte:

**AREA 1 — Tenants & Platform** (le fondamenta)
- `tenants` — Il business (barbershop). Nome, subdomain, stato, config branding, feature flags
- `locations` — Sedi fisiche del tenant. Indirizzo, telefono, coordinate GPS, orari default
- `subscription_plans` — I 3 tier (Starter, Growth, Pro). Tabella globale, no `tenant_id`

**AREA 2 — Auth & Staff**
- `profiles` — Profilo esteso di ogni utente autenticato (staff + clienti con account OTP). Collegato a `auth.users`
- `staff_members` — Dettaglio staff: ruolo nel tenant, stato attivo/inattivio, bio, foto
- `staff_locations` — Ponte N:N: staff assegnato a 1+ sedi

**AREA 3 — Catalogo (Servizi + Prodotti)**
- `services` — Catalogo servizi. Nome, prezzo, durata, categoria, ordine visualizzazione, attivo sì/no
- `staff_services` — Ponte N:N: quali servizi offre ogni staff member
- `products` — Catalogo prodotti. Nome, marca, prezzo vendita, prezzo costo, SKU, foto, categoria
- `product_inventory` — Giacenza per prodotto per sede. Quantità attuale, soglia alert scorta bassa
- `inventory_movements` — (v2) Log movimenti magazzino: carico, scarico, rettifica, reso

**AREA 4 — Calendario & Appuntamenti**
- `working_hours` — Orari settimanali ricorrenti per staff. Giorno settimana + slot multipli
- `working_hour_overrides` — Eccezioni: ferie, chiusure, orari speciali per data specifica
- `appointments` — L'appuntamento. Data, ora, staff, cliente, sede, stato, tipo (online/walk_in), pagamento
- `appointment_services` — Ponte: servizi nell'appuntamento con prezzo snapshot
- `appointment_products` — Ponte: prodotti venduti/usati con prezzo snapshot e quantità

**AREA 5 — Clienti & CRM**
- `clients` — Il cliente nel CRM. Nome, telefono, email, preferenze, consensi GDPR, `profile_id` (nullable)
- `client_notes` — Note private del barbiere. Sempre private, mai visibili al cliente (GDPR)

**AREA 6 — Loyalty & Gamification**
- `loyalty_configs` — Config loyalty del tenant: template, formula punti, soglia streak, attivo sì/no
- `rewards` — Catalogo ricompense. Nome, costo punti, tipo, attiva sì/no. Max 6
- `client_loyalty` — Stato loyalty del cliente. Punti, streak, tier, data ultima visita
- `loyalty_transactions` — Log ogni operazione punti: guadagno, riscatto, bonus, import
- `reward_redemptions` — Riscatti effettuati. Cliente, reward, punti spesi, staff che conferma
- `tier_configs` — (v2) Config 4 livelli per tenant: soglie, benefici ON/OFF
- `badges` — (v2) Catalogo badge del tenant
- `client_badges` — (v2) Badge sbloccati per cliente
- `challenges` — (v2) Sfide temporanee

**AREA 7 — Messaggistica & Notifiche**
- `message_templates` — Template messaggi con placeholder. Tipo: reminder, win_back, review_request
- `messages_log` — Log messaggi inviati. Destinatario, canale, tipo, stato, costo

**AREA 8 — Churn Detection & Analytics**
- `client_analytics` — Metriche calcolate: frequenza media, giorni da ultima visita, spesa totale, VIP score, semaforo churn, spesa prodotti

**AREA 9 — Recensioni**
- `review_requests` — Richiesta recensione inviata. Link Google Reviews, stato, data

**AREA 10 — Admin & Platform**
- `admin_users` — Utenti admin piattaforma (superadmin, non legati a tenant)
- `tenant_activity_log` — Metriche aggregate per tenant: ultimo accesso, appuntamenti mese, clienti, revenue

### Regole architetturali

1. **Ogni tabella ha `tenant_id`** (tranne `tenants`, `subscription_plans`, `admin_users`, `auth.users`)
2. **Supabase Auth gestisce gli utenti** — `auth.users` per auth, `profiles` per dati estesi
3. **RLS policy obbligatorie** su ogni tabella — nessun dato esposto senza policy
4. **Soft delete** dove ha senso — `deleted_at` su clienti e appuntamenti, mai hard delete
5. **UUID** come primary key ovunque — compatibile con Supabase, no collision, no enumeration
6. **Prezzo snapshot** negli appuntamenti — lo storico non cambia se il barbiere aggiorna i prezzi
7. **CRM = fonte di verità unica** — la loyalty funziona con e senza PWA
8. **Note barbiere SEMPRE private** — tabella separata, GDPR-compliant
9. **Giacenza per sede** — `product_inventory` collegata a `products` + `locations`
10. **Estensibilità senza riscrittura** — colonne v2 (streak, tier, badge) presenti con default in v1

### Riepilogo tabelle

| Fase | Tabelle | Di cui globali (no tenant_id) |
|------|---------|-------------------------------|
| **v1 (MVP)** | 28 | 3 (subscription_plans, admin_users, auth.users) |
| **v2 (Growth)** | +5 (tier_configs, badges, client_badges, challenges, inventory_movements) | — |
| **Totale** | 33 | 3 |
